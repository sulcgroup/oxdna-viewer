...
function parseMMCIF(text: string): pdbinfowrapper {
    ...
    while (li < lines.length) {
        ...
        if (trimmed === '' || trimmed[0] === '#') { li++; continue; }
        const tok = tokenizeLine(trimmed);
        for (const t of tok) allTokens.push(t);
        li++;
    }
        ...
        if (tok === 'loop_') {
            ...
            if (loopCols.length > 0 && loopCols[0].startsWith('_atom_site.')) {
                ...
                while (ti < allTokens.length) {
                    ...
                    if (t0 === 'loop_' || t0.startsWith('data_')) break;
                    if (t0.length > 0 && t0[0] === '_') break;
                    // Ensure we have enough tokens for a full row
                    if (ti + colCount > allTokens.length) break;
                    // Peek: check none of the next colCount tokens are keywords
                    let rowOk = true;
                    for (let k = 0; k < colCount; k++) {
                        const tk = allTokens[ti + k];
                        if (tk === 'loop_' || tk.startsWith('data_')) { rowOk = false; break; }
                        if (tk.length > 0 && tk[0] === '_') { rowOk = false; break; }
                    }
                    if (!rowOk) break;
            ...
            while (ti < allTokens.length) {
                const t0 = allTokens[ti];
                if (t0 === 'loop_' || t0.startsWith('data_') || (t0.length > 0 && t0[0] === '_')) break;
                ti++;
            }
        } else {
            ti++;
    ...
    const atomArray: pdbatom[] = [];
    for (const row of atomRows) {
        // Keep only ATOM records (not HETATM)
        if (gi !== undefined && row[gi] !== 'ATOM') continue;
        // Keep only model 1
        if (modelI !== undefined && row[modelI] !== '1') continue;
        ...
        if (atom.altLoc === '.' || atom.altLoc === '?') atom.altLoc = '';
        atom.resType    = compIdI  !== undefined ? row[compIdI]  : '';
        atom.chainID    = chainIdI !== undefined ? row[chainIdI] : 'A';
        if (atom.chainID === '.' || atom.chainID === '?') atom.chainID = 'A';
        atom.pdbResIdent = seqIdI !== undefined ? row[seqIdI] : '';
        if (atom.pdbResIdent === '.' || atom.pdbResIdent === '?') atom.pdbResIdent = '0';
    ...
    const chainAtomIndx: {[cid: string]: number} = {};
    const chainResKeys: {[cid: string]: string[]} = {};
    const chainResMap: {[cid: string]: {[rk: string]: pdbresidue}} = {};
