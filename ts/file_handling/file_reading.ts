...
function readUNFString(s: string) {
    ...
    let sys = new System(sysStartId, elements.getNextId());
    sys.label = data.name;
    let sidCounter = 0;
    ...
    data.naStrands.forEach((s) => {
        ...
        let strand = new NucleicAcidStrand(s.id, sys);
        
        strand.label = s.name
        let naType;
        switch(s.naType) {
            case 'DNA': naType = DNANucleotide; break;
            case 'RNA': naType = RNANucleotide; break;
            case 'XNA':
                ...
                naType = RNANucleotide; 
                break;
            default:
                notify(`ERROR: ${s.naType} is not a recognized strand type.  Skipping.`)
                return;
        }
        ...
        s.nucleotides.forEach((n) => {
            let e: Nucleotide;
            e = new naType(elements.getNextId(), strand);
    ...
    data.naStrands.forEach((s, i) => {
        let strand = sys.strands[i];

        //set strand ends
        strand.end3 = elements.get(newElementIds.get(s.threePrimeId));
        strand.end5 = elements.get(newElementIds.get(s.fivePrimeId));

        s.nucleotides.forEach((n) => {
            let e = elements.get(newElementIds.get(n.id)) as unknown as Nucleotide; //we know it's a nucleotide because it's part of a naStrand object.
            
            //set interactions
            e.n3 = elements.get(newElementIds.get(n.next));
            e.n5 = elements.get(newElementIds.get(n.prev));
            if (n.pair) {
                e.pair = elements.get(newElementIds.get(n.pair)) as unknown as Nucleotide;
            }
        });
    });
