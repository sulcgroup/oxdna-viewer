...
function addPDBToScene(strands: pdbchain[]) {
    ...
    for (let i: number = 0; i < strandnum; i++) {
        let strand = strands[i];

        if (strand.strandtype == 'pro') {
            let currentStrand: Peptide = pdbsystem.addNewPeptideStrand();
            strand.residues.forEach(res => {
                if (res.type == 'pro') {
            ...
            let currentStrand: NucleicAcidStrand = pdbsystem.addNewNucleicAcidStrand();
            strand.residues.forEach(res => {
    ...
    for (let i: number = 0; i < strandnum; i++) {
        ...
        if (strand.isPeptide()) {
            pdbstrand.residues.forEach(res => {
                if (res.type == 'pro') {
                    let AA = <AminoAcid>strand.getMonomers()[nextElementId];
                    nextElementId++;
