...
abstract class Nucleotide extends BasicElement {
    ...
    calcPositions(p: THREE.Vector3, a1: THREE.Vector3, a3: THREE.Vector3) {
        ...
        const rotationCon1 = new THREE.Quaternion().setFromUnitVectors(
            new THREE.Vector3(0, 1, 0),
            p.clone().sub(con1).normalize());
        //const rotationCon1 = new THREE.Quaternion(1, 0, 0, 0)
        //const rotationCon2 = new THREE.Quaternion(0, 1, 0, 0)
        const rotationCon2 = new THREE.Quaternion().setFromUnitVectors(
            new THREE.Vector3(0, 1, 0),
            p.clone().sub(ns).normalize());
