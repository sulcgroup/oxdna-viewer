...
abstract class Strand {
   ...
   */
  forEach(
    callbackfn: (value: BasicElement, index: number) => void,
    reverse?: boolean,
    condition?: (value: BasicElement, index: number) => boolean
  ) {
   ...
   */
  filter(
    callbackfn: (value: BasicElement, index: number) => boolean,
    reverse?: boolean
  ) {
    let list = [];
    this.forEach((e, i) => {
...
class NucleicAcidStrand extends Strand {
  ...
  createBasicElement(id?: number) {
    if (RNA_MODE) return new RNANucleotide(id, this);
    else return new DNANucleotide(id, this);
  }

  createBasicElementTyped(type: string, id?: number) {
    if (type == "rna") return new RNANucleotide(id, this);
    else if (type == "dna") return new DNANucleotide(id, this);
  }
