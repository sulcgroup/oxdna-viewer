 ...
 * Running this will allow you to transform selections on the screen.
 * Heavily rewritten by Joakim Bohlin sfor use in oxView
 */
...
THREE.TransformControls = function ( camera, domElement ) {
	...
	// Attach to selected bases
	this.show = function () {
		this.visible = true;

		currentPosition = new THREE.Vector3();
		selectedBases.forEach(e => currentPosition.add(e.getInstanceParameter3("cmOffsets")));
	...
	this.updateMatrixWorld = function () {
		...
		if ( this.visible ) {
			...
			worldQuaternion.copy(quaternion);
	...
	this.pointerDown = function ( pointer ) {
		...
		selectedBases.forEach(e => position.add(e.getInstanceParameter3("cmOffsets")));
		...
		if ( ( pointer.button === 0 || pointer.button === undefined ) && this.axis !== null ) {
			...
			if ( planeIntersect ) {
				...
				if ( space === 'local' && this.mode === 'rotate' ) {
					...
					if ( this.axis === 'X' && snap ) rotation.x = Math.round( rotation.x / snap ) * snap;
					if ( this.axis === 'Y' && snap ) rotation.y = Math.round( rotation.y / snap ) * snap;
					if ( this.axis === 'Z' && snap ) rotation.z = Math.round( rotation.z / snap ) * snap;

				}


				positionStart.copy( position );
				quaternion.copy( new THREE.Quaternion().setFromEuler(new THREE.Euler(rotation.x, rotation.y, rotation.z, 'XYZ')));
				quaternionStart.copy(quaternion);
	...
	this.pointerMove = function ( pointer ) {
		...
		if ( !this.visible || axis === null || this.dragging === false || ( pointer.button !== undefined && pointer.button !== 0 ) ) return;
		...
		if ( mode === 'translate' ) {
			...
			position.copy( offset ).add( positionStart );
			...
			if ( this.translationSnap ) {
				...
				if ( space === 'local' ) {
					...
					position.applyQuaternion( _tempQuaternion.copy( quaternionStart ).inverse() );
			...
			if ( space === 'local' && axis !== 'E' && axis !== 'XYZE' ) {
				...
				quaternion.copy( quaternionStart );
				quaternion.multiply( _tempQuaternion.setFromAxisAngle( rotationAxis, rotationAngle ) ).normalize();

			} else {

				rotationAxis.applyQuaternion( parentQuaternionInv );
				quaternion.copy( _tempQuaternion.setFromAxisAngle( rotationAxis, rotationAngle ) );
				quaternion.multiply( quaternionStart ).normalize();
		...
		currentQuaternion.copy(quaternion);
	...
	this.pointerUp = function ( pointer ) {
		...
		if ( this.dragging && ( this.axis !== null ) ) {
			...
			// Add to undo history
			var totalTranslation = currentPosition.clone().sub(positionStart);
			var totalRotation = currentQuaternion.clone().multiply(quaternionStart.clone().inverse());
			editHistory.add(new RevertableTransformation(selectedBases, totalTranslation, totalRotation, positionStart));   
		}
