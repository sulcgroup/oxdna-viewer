...
/**
 * Enhanced Library UI with Sync Status Indicators and Conflict Resolution
 * Implements three-tier structure: Local Projects, Shared (combined), and enhanced sync features
 */

// Sync status types
type SyncStatus = 'local' | 'synced' | 'out-of-sync' | 'conflict' | 'public-synced' | 'public-out-of-sync';
...
async function getProjectSyncStatus(localProject: any): Promise<SyncState> {
  try {
    ...
    if (localProject.publicSourceId && (!localProject.isSynced || !localProject.syncedProjectId)) {
      try {
        const remotePublicCommits = await (window as any).getPublicProjectCommits(localProject.publicSourceId);
        ...
        const localIds = new Set(localProject.commits.map((c: any) => c.commitId));
        const missing = remotePublicCommits.some((rc: any) => !localIds.has(rc.id));
        if (missing) {
          return {
    ...
    const remoteCommits = await (window as any).getProjectCommits(localProject.syncedProjectId);
...
function showConflictResolutionDialog(project: any, localState: any, remoteState: any) {
  const content = `
    <div>
  ...
  (window as any).Metro.dialog.create({
...
async function pullRemoteProject(projectIdOrRemote: any, remoteProjectId?: string) {
  try {
    ...
    let remoteId: string | undefined = remoteProjectId;
    let remoteObj: any | undefined = undefined;
    ...
    if (typeof projectIdOrRemote === 'object' && projectIdOrRemote !== null) {
      ...
      localId = projectIdOrRemote as string;
    }
    ...
    // Get remote project commits
    const remoteCommits = await (window as any).getProjectCommits(remoteId);
    ...
    for (const commit of remoteCommits) {
      // Get commit data
      const commitData = await (window as any).readCommitData(remoteId, commit.id);
      if (!commitData) {
    ...
    const localProject = await (window as any).DexieDB.structureData.get(localId);
    if (!localProject) {
      const newProject = {
        id: remoteId,
        ...
        date: Date.now(),
        branches: { main: localCommits.map((c: any) => c.commitId) },
        isSynced: true,
...
interface EntryType {
  id: string;
  // commitId is optional because older entries might not include it.
  commits: { data: ArrayBuffer; commitName?: string; commitId?: string; parent?: string }[]; // Renamed from 'structure' to 'commits'
...
async function renderLocalProjectsSection(container: HTMLElement) {
  ...
  const localProjects = await (window as any).DexieDB.structureData.toArray();
  ...
  const remoteOnlyProjects = remoteProjects.filter((remoteProject: any) => {
  ...
  async function renderProjectsPage(page: number) {
...
async function renderSharedSection(container: HTMLElement) {
  const section = document.createElement("div");
  ...
  function renderSharedPage(page: number) {
    sharedRow.innerHTML = "";
...
async function createProjectCard(project: any): Promise<HTMLElement> {
  const cell = document.createElement("div");
  cell.className = "cell-4";
  ...
  if (project.commits && project.commits.length > 0) {
    latestCommit = project.commits.reduce((prev: any, cur: any) => {
      ...
      if (!prev.createdAt && !cur.createdAt) return cur;
  ...
  if (project.isSynced && project.syncedProjectId) {
    ...
    deleteRemoteBtn.addEventListener("click", async (event) => {
      ...
      if (confirm(`Are you sure you want to delete the remote copy of "${project.structureName}"? This will delete it from the cloud but keep your local copy.`)) {
        const success = await (window as any).deleteSyncedProject(project.syncedProjectId);
        if (success) {
          // Update local project to mark as unsynced
          await (window as any).DexieDB.structureData.update(project.id, {
            isSynced: false,
    ...
    togglePublicBtn.addEventListener("click", async (event) => {
      event.preventDefault();
      event.stopPropagation();
      const result = await (window as any).toggleProjectPublic(project.syncedProjectId);
      if (result.success) {
        ...
        if (notify) {
          notify.create(
            result.isPublic ? "Project is now public" : "Project is now private",
            null,
            { cls: "success", keepOpen: false }
          );
        } else {
...
async function createRemoteProjectCard(project: any): Promise<HTMLElement> {
  const cell = document.createElement("div");
  cell.className = "cell-4";
...
function createSharedCommitCard(commit: any): HTMLElement {
  const cell = document.createElement("div");
...
async function refreshPublicClonedProject(localProject: any): Promise<void> {
  try {
    const notify = (window as any).Metro?.notify;
    if (notify) {
    ...
    if (notify) notify.create("Public project refreshed", null, { cls: "success", keepOpen: false });
    refreshLibraryCards();
  } catch (e) {
    console.error("refreshPublicClonedProject error", e);
    const notify = (window as any).Metro?.notify;
    if (notify) notify.create("Failed to refresh", null, { cls: "alert", keepOpen: false });
    else alert("Failed to refresh public project");
  }
}
...
async function syncProjectToCloud(projectId: string, projectName: string): Promise<void> {
  try {
    ...
    const localProject = await (window as any).DexieDB.structureData.get(projectId);
    if (!localProject) {
...
async function createNew(): Promise<void> {
  try {
    ...
    if (!nameElement || nameElement.value === "") {
      ...
      const newId = (window as any).createId();
