...
async function initBranchSelection(structureId?: string) {
  ...
  if (!structureId) {
    structureId = (window as any).branchSelectionStructureId;
  }
  ...
  try {
    currentStructure = await (window as any).DexieDB.structureData.get(structureId);
...
function selectCommit(commit: Commit, nodeElement: HTMLElement) {
  ...
  const generateBtn = document.getElementById('generate-link-btn') as HTMLButtonElement;
  if (generateBtn) {
...
function setupEventListeners() {
  ...
  if (cancelBtn) {
    cancelBtn.addEventListener('click', () => {
      (view as any).closeWindow('branchSelectionWindow');
    });
  }
...
async function handleGenerateLink() {
  ...
  if (!generateBtn || !shareResult) return;
...
async function generatePermanentLink(commit: Commit, structure: Structure) {
...
async function updateCommitShareInfo(structureId: string, commitId: string, shareInfo: any) {
  try {
...
function displayShareResult(shareUrl: string, expiresAt: Date) {
  const shareResult = document.getElementById('share-result');
...
(window as any).initBranchSelection = initBranchSelection;
