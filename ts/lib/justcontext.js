...
function hideAll(e){
	let visible = false;
	Array.from(document.querySelectorAll(".jctx")).forEach((x,i)=>{if(x.style.display !='none') visible=true});
	if(visible) Array.from(document.querySelectorAll(".jctx")).forEach((x,i)=>{fadeElement(x,'hide')})
};
...
onDOMReady(function(){
	...
	Array.from(document.querySelectorAll(".jctx li")).forEach((x,i)=>{
		addListener(x,"click",function(event){
			console.log(`Diff: ${event.movementX+event.movementY}`);
			if(eval("typeof(handleMenuAction)==typeof(Function)")&&!x.classList.contains("disabled")) {
				handleMenuAction(x.getAttribute("data-action"));
			}
			fadeElement(x.parentElement,'hide')
		})
	});
	addListener(document,"mousedown",hideAll);
