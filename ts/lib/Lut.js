...
THREE.Lut.prototype = {
    ...
    setLegendLabels: function(parameters, callback) {
        ...
        contextTitle.fillText(
			this.legend.labels.title.toString() + this.legend.labels.um.toString(),
			borderThickness,
			this.legend.labels.fontsize + borderThickness
		);
		...
		spriteTitle.scale.set(0.2, 0.1, 0);
		spriteTitle.position.set(
			//this.legend.position.x,
			(this.legend.position.x - (this.legend.dimensions.height * 0.5) + 0.1),
        ...
        if (this.legend.labels.ticks > 0) {
            ...
            for (var i = 0; i < this.legend.labels.ticks; i++) {
                ...
                var spriteMaterialTick = new THREE.SpriteMaterial({
                    map: txtTick
                });
