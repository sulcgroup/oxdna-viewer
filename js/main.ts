...
function nextConfig() {
    for (let i = 0; i < systems.length; i++) {
        ...
        for (let i = datnum * datlen; i < trajlen; i++) {
            ...
            //last, add the sugar-phosphate bond since its not done for the first nucleotide in each strand
            if (current_nucleotide.neighbor3 != null) {
                let x_sp = (x_bb + current_nucleotide.neighbor3.visual_object.position.x) / 2,
                    y_sp = (y_bb + current_nucleotide.neighbor3.visual_object.position.y) / 2,
                    z_sp = (z_bb + current_nucleotide.neighbor3.visual_object.position.z) / 2;
    ...
    if (lutCols.length > 0) {
        if (lutColsVis) {
            for (let i = 0; i < nucleotides.length; i++) {
                ...
                let back_Mesh: THREE.Object3D = nucleotides[i].visual_object.children[0];
                let nuc_Mesh: THREE.Object3D = nucleotides[i].visual_object.children[1];
                let con_Mesh: THREE.Object3D = nucleotides[i].visual_object.children[2];
                let sp_Mesh: THREE.Object3D = nucleotides[i].visual_object.children[4];
