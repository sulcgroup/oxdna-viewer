    ...
    if (showColliders) {
        if (sphereMode) {
            ...
            if (clusterData.length > 0)
                standaloneColliderViz = new SphereClusterColliderVisualizer(clusterData);
        }
        else {
...
function buildStandaloneClusterData() {
    const m = new Map();
    elements.forEach(e => {
        if (e.clusterId < 0)
            return;
        if (!m.has(e.clusterId))
            m.set(e.clusterId, []);
    ...
    return [...m.values()].map(elems => {
        ...
        const conPoints = [];
        for (const e of elems) {
            if (e.n3 && e.n3.clusterId !== cid)
                conPoints.push({ from: e });
            if (e.n5 && e.n5.clusterId !== cid)
                conPoints.push({ from: e });
            forceHandler.getTraps().forEach((t) => {
                if (t.particle === e)
                    conPoints.push({ from: e });
...
class SphereClusterColliderVisualizer {
    ...
    constructor(clusters) {
        ...
        for (const c of clusters) {
            ...
            colorBuffer[idx * 3] = col.r;
            colorBuffer[idx * 3 + 1] = col.g;
            colorBuffer[idx * 3 + 2] = col.b;
            rotBuffer[idx * 4 + 3] = 1;
