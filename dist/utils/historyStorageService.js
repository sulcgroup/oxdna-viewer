...
export async function saveStructure() {
    try {
        ...
        if (urlParams.has("structureId")) {
            ...
            if (id) {
                const old = await window.DexieDB.structureData.get(id);
                if (old) {
                    const newDataArr = [
                        ...old.structure,
        ...
        if (urlParams.has("structureId")) {
            if (urlParams.has("commit")) {
                ...
                if (id) {
                    ...
                    const compData = new Uint8Array(data.find((i) => i.commitName === urlParams.get("commit")).data);
...
async function viewHistory() {
    ...
    try {
        ...
        const entry = await window.DexieDB.structureData.get(structureId);
        if (!entry) {
