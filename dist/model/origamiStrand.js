class OrigamiStaple extends Strand {
    ...
    isFake;
    elements;
    constructor(id, system, sequence, fake = false, velocity = 10, direction = new THREE.Vector3(0, 0, 1)) {
        super(id, system);
    ...
    static fake(staple, startIdx, endIdx) {
        ...
        if (startIdx != 0 && endIdx != staple.sequence.length - 1) {
            ...
            fakes.push(new OrigamiStaple(staple.id, staple.system, fakeSequence1, true));
            fakes.push(new OrigamiStaple(staple.id, staple.system, fakeSequence2, true));
        }
        else if (startIdx != 0) {
            const fakeSequence = staple.sequence.slice(0, startIdx);
            fakes.push(new OrigamiStaple(staple.id, staple.system, fakeSequence, true));
        }
        else {
