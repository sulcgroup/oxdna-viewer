 ...
 */
function buildMutualTrapPairMap() {
    const pairMap = new Map();
    const traps = forceHandler.getTraps().filter(f => f instanceof MutualTrap);
 ...
 */
function generateExtendedDotBracketPlus(useMutualTraps = false) {
    if (systems.length === 0) {
        notify("No system loaded", "alert");
        return null;
    }
    const sys = systems[systems.length - 1];
    const naStrands = sys.strands.filter(s => s.isNucleicAcid());
    ...
    naStrands.forEach(strand => {
        const monomers = strand.getMonomers();
    ...
    const bracketPairs = [['(', ')'], ['[', ']'], ['{', '}']];
    const dbArray = new Array(n).fill('.');
    ...
    const pairLevels = new Array(pairs.length).fill(-1);
...
function generateDBPText() {
    const useMutualTraps = document.getElementById('dbpUseMutualTraps')?.checked || false;
    const result = generateExtendedDotBracketPlus(useMutualTraps);
    if (!result)
        return;
    const textarea = document.getElementById('dbpOutput');
    if (textarea) {
...
function sendToDBPViewIframe() {
    ...
    const iframe = document.getElementById('dbpViewIframe');
