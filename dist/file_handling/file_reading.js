...
function readUNFfile(file) {
    let reader = new FileReader();
    reader.onload = (e) => {
        readUNFString(e.target.result);
    };
    reader.readAsText(file);
}
function readUNFString(s) {
    ...
    data.naStrands.forEach((s, i) => {
        ...
        s.nucleotides.forEach((n) => {
            let e = elements.get(newElementIds.get(n.id)); //we know it's a nucleotide because it's part of a naStrand object.
            //set interactions
